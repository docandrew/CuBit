...
-- Keyboard Driver
-------------------------------------------------------------------------------
with CuBit.Drivers.Keyboard.Scancodes;

package body CuBit.Drivers.Keyboard
    with SPARK_Mode => On
is
    ...
    -- readKey
    ---------------------------------------------------------------------------
    procedure readKey (code : Unsigned_8) is
        scode : ScanCode;
    begin
        if code < 128 then
            ...
            case scode is
                ...
                when SCAN_CAPS_LOCK =>
                    null;
                when others =>
                    debugPrint ("" & asciiOf (scode, shifted, caps));
            end case;
        else
            -- Key Release
            code := code - 128;
    ...
    begin
        loop
            msg := CuBit.Messages.recvMsg (sender);
            
            readKey (Unsigned_8(msg));
            
            ok := CuBit.Messages.replyMsg (sender, 1);
        end loop;
    end mainLoop;

end CuBit.Drivers.Keyboard;
