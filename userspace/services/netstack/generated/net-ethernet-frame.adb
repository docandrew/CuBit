...
package body Net.Ethernet.Frame
with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, Frame_Size, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (others => <>));
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, Frame_Size, First - 1, First - 1, Ctx.Buffer, (others => <>));
   end Reset;
   ...
   with
     Pre =>
       ...
       and then Net.Ethernet.Frame.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.Ethernet.Frame.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then Net.Ethernet.Frame.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.Ethernet.Frame.Available_Space (Ctx, Fld)
       and then (if Net.Ethernet.Frame.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.Ethernet.Frame.Valid_Value (Fld, Val)
       and then Net.Ethernet.Frame.Valid_Size (Ctx, Fld, Net.Ethernet.Frame.Field_Size (Ctx, Fld))
       and then Net.Ethernet.Frame.Available_Space (Ctx, Fld) >= Net.Ethernet.Frame.Field_Size (Ctx, Fld)
       and then Net.Ethernet.Frame.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.Ethernet.Frame.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Destination (Ctx : in out Context; Val : Net.Ethernet.Address) is
   begin
      Set_Scalar (Ctx, F_Destination, Net.Ethernet.To_Base_Integer (Val));
   end Set_Destination;

   procedure Set_Source (Ctx : in out Context; Val : Net.Ethernet.Address) is
   begin
      Set_Scalar (Ctx, F_Source, Net.Ethernet.To_Base_Integer (Val));
   end Set_Source;

   procedure Set_Type_Length_TPID (Ctx : in out Context; Val : Net.Ethernet.Type_Length) is
   begin
      Set_Scalar (Ctx, F_Type_Length_TPID, Net.Ethernet.To_Base_Integer (Val));
   end Set_Type_Length_TPID;

   procedure Set_TPID (Ctx : in out Context; Val : Net.Ethernet.TPID) is
   begin
      Set_Scalar (Ctx, F_TPID, Net.Ethernet.To_Base_Integer (Val));
   end Set_TPID;

   procedure Set_TCI (Ctx : in out Context; Val : Net.Ethernet.TCI) is
   begin
      Set_Scalar (Ctx, F_TCI, Net.Ethernet.To_Base_Integer (Val));
   end Set_TCI;

   procedure Set_Type_Length (Ctx : in out Context; Val : Net.Ethernet.Type_Length) is
   begin
      Set_Scalar (Ctx, F_Type_Length, Net.Ethernet.To_Base_Integer (Val));
   end Set_Type_Length;

   procedure Set_Ether_Type (Ctx : in out Context; Val : Net.Ethernet.Ether_Type) is
   begin
      Set_Scalar (Ctx, F_Ether_Type, Net.Ethernet.To_Base_Integer (Val));
   end Set_Ether_Type;
   ...
   begin
      ...
      pragma Assert (Net.Ethernet.Frame.Valid_Length (Ctx, Net.Ethernet.Frame.F_Payload, Length));
