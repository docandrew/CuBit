...
generic
   ...
   with function Valid (Element : Net.RFLX_Types.Base_Integer) return Boolean;
   with function To_Actual (Element : Net.RFLX_Types.Base_Integer) return Element_Type;
   with function To_Base_Int (Element : Element_Type) return Net.RFLX_Types.Base_Integer;
package Net.RFLX_Scalar_Sequence
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   with
     ...
     Post =>
       Has_Buffer (Ctx)
       and Sequence_Last (Ctx) = Sequence_Last (Ctx)'Old + Net.RFLX_Types.Bit_Index (Element_Size)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old;
   ...
   with
     Pre =>
       Valid (Ctx)
       and then Sequence_Last (Ctx) >= (Ctx.First + Net.RFLX_Types.Bit_Index (Element_Size)) - 1;
   ...
   with
     Pre =>
       ...
       and then Available_Space (Ctx) >= Net.RFLX_Types.Bit_Index (Element_Size),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx)
       and Sequence_Last (Ctx) = Sequence_Last (Ctx)'Old + Net.RFLX_Types.Bit_Index (Element_Size)
...
private
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First) is
      record
         ...
         First_Element : Net.RFLX_Types.Base_Integer := Net.RFLX_Types.Base_Integer'First;
         Next_Element : Net.RFLX_Types.Base_Integer := Net.RFLX_Types.Base_Integer'First;
      end record
   with
     Dynamic_Predicate =>
   ...
   function Has_Element (Ctx : Context) return Boolean is
     (Ctx.State = S_Valid
      and Ctx.Last - Ctx.Sequence_Last >= Net.RFLX_Types.Bit_Index (Element_Size));
