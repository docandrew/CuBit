...
is
   ...
   with
     Pre =>
       ...
       and then Net.ICMP.Message.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.ICMP.Message.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then Net.ICMP.Message.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.ICMP.Message.Available_Space (Ctx, Fld)
       and then (if Net.ICMP.Message.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.ICMP.Message.Valid_Value (Fld, Val)
       and then Net.ICMP.Message.Valid_Size (Ctx, Fld, Net.ICMP.Message.Field_Size (Ctx, Fld))
       and then Net.ICMP.Message.Available_Space (Ctx, Fld) >= Net.ICMP.Message.Field_Size (Ctx, Fld)
       and then Net.ICMP.Message.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.ICMP.Message.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Tag (Ctx : in out Context; Val : Net.ICMP.Tag) is
   begin
      Set_Scalar (Ctx, F_Tag, Net.ICMP.To_Base_Integer (Val));
   end Set_Tag;

   procedure Set_Code_Destination_Unreachable (Ctx : in out Context; Val : Net.ICMP.Code_Destination_Unreachable) is
   begin
      Set_Scalar (Ctx, F_Code_Destination_Unreachable, Net.ICMP.To_Base_Integer (Val));
   end Set_Code_Destination_Unreachable;

   procedure Set_Code_Redirect (Ctx : in out Context; Val : Net.ICMP.Code_Redirect) is
   begin
      Set_Scalar (Ctx, F_Code_Redirect, Net.ICMP.To_Base_Integer (Val));
   end Set_Code_Redirect;

   procedure Set_Code_Time_Exceeded (Ctx : in out Context; Val : Net.ICMP.Code_Time_Exceeded) is
   begin
      Set_Scalar (Ctx, F_Code_Time_Exceeded, Net.ICMP.To_Base_Integer (Val));
   end Set_Code_Time_Exceeded;

   procedure Set_Code_Zero (Ctx : in out Context; Val : Net.ICMP.Code_Zero) is
   begin
      Set_Scalar (Ctx, F_Code_Zero, Net.ICMP.To_Base_Integer (Val));
   end Set_Code_Zero;

   procedure Set_Checksum (Ctx : in out Context; Val : Net.ICMP.Checksum) is
   begin
      Set_Scalar (Ctx, F_Checksum, Net.ICMP.To_Base_Integer (Val));
   end Set_Checksum;

   procedure Set_Gateway_Internet_Address (Ctx : in out Context; Val : Net.ICMP.Gateway_Internet_Address) is
   begin
      Set_Scalar (Ctx, F_Gateway_Internet_Address, Net.ICMP.To_Base_Integer (Val));
   end Set_Gateway_Internet_Address;

   procedure Set_Identifier (Ctx : in out Context; Val : Net.ICMP.Identifier) is
   begin
      Set_Scalar (Ctx, F_Identifier, Net.ICMP.To_Base_Integer (Val));
   end Set_Identifier;

   procedure Set_Pointer (Ctx : in out Context; Val : Net.ICMP.Pointer) is
   begin
      Set_Scalar (Ctx, F_Pointer, Net.ICMP.To_Base_Integer (Val));
   end Set_Pointer;

   procedure Set_Unused_32 (Ctx : in out Context; Val : Net.ICMP.Unused_32) is
   begin
      Set_Scalar (Ctx, F_Unused_32, Net.ICMP.To_Base_Integer (Val));
   end Set_Unused_32;

   procedure Set_Sequence_Number (Ctx : in out Context; Val : Net.ICMP.Sequence_Number) is
   begin
      Set_Scalar (Ctx, F_Sequence_Number, Net.ICMP.To_Base_Integer (Val));
   end Set_Sequence_Number;

   procedure Set_Unused_24 (Ctx : in out Context; Val : Net.ICMP.Unused_24) is
   begin
      Set_Scalar (Ctx, F_Unused_24, Net.ICMP.To_Base_Integer (Val));
   end Set_Unused_24;

   procedure Set_Originate_Timestamp (Ctx : in out Context; Val : Net.ICMP.Timestamp) is
   begin
      Set_Scalar (Ctx, F_Originate_Timestamp, Net.ICMP.To_Base_Integer (Val));
   end Set_Originate_Timestamp;

   procedure Set_Receive_Timestamp (Ctx : in out Context; Val : Net.ICMP.Timestamp) is
   begin
      Set_Scalar (Ctx, F_Receive_Timestamp, Net.ICMP.To_Base_Integer (Val));
   end Set_Receive_Timestamp;

   procedure Set_Transmit_Timestamp (Ctx : in out Context; Val : Net.ICMP.Timestamp) is
   begin
      Set_Scalar (Ctx, F_Transmit_Timestamp, Net.ICMP.To_Base_Integer (Val));
   ...
   begin
      ...
      pragma Assert (Net.ICMP.Message.Valid_Length (Ctx, Net.ICMP.Message.F_Data, Length));
      Initialize_Data_Private (Ctx, Length);
