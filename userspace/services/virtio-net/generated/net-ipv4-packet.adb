...
is
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.IPv4.Packet.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.IPv4.Packet.Available_Space (Ctx, Fld)
       and then (if Net.IPv4.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Packet.Valid_Value (Fld, Val)
       and then Net.IPv4.Packet.Valid_Size (Ctx, Fld, Net.IPv4.Packet.Field_Size (Ctx, Fld))
       and then Net.IPv4.Packet.Available_Space (Ctx, Fld) >= Net.IPv4.Packet.Field_Size (Ctx, Fld)
       and then Net.IPv4.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.IPv4.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Version (Ctx : in out Context; Val : Net.IPv4.Version) is
   begin
      Set_Scalar (Ctx, F_Version, Net.IPv4.To_Base_Integer (Val));
   end Set_Version;

   procedure Set_IHL (Ctx : in out Context; Val : Net.IPv4.IHL) is
   begin
      Set_Scalar (Ctx, F_IHL, Net.IPv4.To_Base_Integer (Val));
   end Set_IHL;

   procedure Set_DSCP (Ctx : in out Context; Val : Net.IPv4.DCSP) is
   begin
      Set_Scalar (Ctx, F_DSCP, Net.IPv4.To_Base_Integer (Val));
   end Set_DSCP;

   procedure Set_ECN (Ctx : in out Context; Val : Net.IPv4.ECN) is
   begin
      Set_Scalar (Ctx, F_ECN, Net.IPv4.To_Base_Integer (Val));
   end Set_ECN;

   procedure Set_Total_Length (Ctx : in out Context; Val : Net.IPv4.Total_Length) is
   begin
      Set_Scalar (Ctx, F_Total_Length, Net.IPv4.To_Base_Integer (Val));
   end Set_Total_Length;

   procedure Set_Identification (Ctx : in out Context; Val : Net.IPv4.Identification) is
   begin
      Set_Scalar (Ctx, F_Identification, Net.IPv4.To_Base_Integer (Val));
   ...
   procedure Set_Fragment_Offset (Ctx : in out Context; Val : Net.IPv4.Fragment_Offset) is
   begin
      Set_Scalar (Ctx, F_Fragment_Offset, Net.IPv4.To_Base_Integer (Val));
   end Set_Fragment_Offset;

   procedure Set_TTL (Ctx : in out Context; Val : Net.IPv4.TTL) is
   begin
      Set_Scalar (Ctx, F_TTL, Net.IPv4.To_Base_Integer (Val));
   end Set_TTL;

   procedure Set_Protocol (Ctx : in out Context; Val : Net.Protocol_Numbers.Assigned_Internet_Protocol_Numbers_Enum) is
   begin
      Set_Scalar (Ctx, F_Protocol, Net.Protocol_Numbers.To_Base_Integer (Val));
   end Set_Protocol;

   procedure Set_Header_Checksum (Ctx : in out Context; Val : Net.IPv4.Header_Checksum) is
   begin
      Set_Scalar (Ctx, F_Header_Checksum, Net.IPv4.To_Base_Integer (Val));
   end Set_Header_Checksum;

   procedure Set_Source (Ctx : in out Context; Val : Net.IPv4.Address) is
   begin
      Set_Scalar (Ctx, F_Source, Net.IPv4.To_Base_Integer (Val));
   end Set_Source;

   procedure Set_Destination (Ctx : in out Context; Val : Net.IPv4.Address) is
   begin
      Set_Scalar (Ctx, F_Destination, Net.IPv4.To_Base_Integer (Val));
   end Set_Destination;
   ...
   procedure Set_Options (Ctx : in out Context; Seq_Ctx : Net.IPv4.Options.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (Net.IPv4.Options.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Options, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      Net.IPv4.Options.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Options;
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Packet.Valid_Length (Ctx, Net.IPv4.Packet.F_Options, Length)
       and then RFLX_Types.To_Length (Net.IPv4.Packet.Available_Space (Ctx, Net.IPv4.Packet.F_Options)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   begin
      ...
      pragma Assert (Net.IPv4.Packet.Valid_Length (Ctx, Net.IPv4.Packet.F_Payload, Length));
      Initialize_Payload_Private (Ctx, Length);
   end Generic_Set_Payload;

   procedure Switch_To_Options (Ctx : in out Context; Seq_Ctx : out Net.IPv4.Options.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Options);
   ...
   begin
      ...
      Net.IPv4.Options.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Options;

   procedure Update_Options (Ctx : in out Context; Seq_Ctx : in out Net.IPv4.Options.Context) is
      Valid_Sequence : constant Boolean := Net.IPv4.Packet.Complete_Options (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      Net.IPv4.Options.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
