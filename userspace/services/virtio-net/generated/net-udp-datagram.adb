...
is
   ...
   with
     Pre =>
       ...
       and then Net.UDP.Datagram.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.UDP.Datagram.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       Net.UDP.Datagram.Has_Buffer (Ctx)
       and then Net.UDP.Datagram.Valid_Next (Ctx, Fld)
       and then Net.UDP.Datagram.Valid_Value (Fld, Val)
       and then Net.UDP.Datagram.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.UDP.Datagram.Available_Space (Ctx, Fld)
       and then (if Net.UDP.Datagram.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.UDP.Datagram.Valid_Value (Fld, Val)
       and then Net.UDP.Datagram.Valid_Size (Ctx, Fld, Net.UDP.Datagram.Field_Size (Ctx, Fld))
       and then Net.UDP.Datagram.Available_Space (Ctx, Fld) >= Net.UDP.Datagram.Field_Size (Ctx, Fld)
       and then Net.UDP.Datagram.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.UDP.Datagram.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Source_Port (Ctx : in out Context; Val : Net.UDP.Port) is
   begin
      Set_Scalar (Ctx, F_Source_Port, Net.UDP.To_Base_Integer (Val));
   end Set_Source_Port;

   procedure Set_Destination_Port (Ctx : in out Context; Val : Net.UDP.Port) is
   begin
      Set_Scalar (Ctx, F_Destination_Port, Net.UDP.To_Base_Integer (Val));
   end Set_Destination_Port;

   procedure Set_Length (Ctx : in out Context; Val : Net.UDP.Length) is
   begin
      Set_Scalar (Ctx, F_Length, Net.UDP.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Checksum (Ctx : in out Context; Val : Net.UDP.Checksum) is
   begin
      Set_Scalar (Ctx, F_Checksum, Net.UDP.To_Base_Integer (Val));
   end Set_Checksum;
   ...
   begin
      ...
      pragma Assert (Net.UDP.Datagram.Valid_Length (Ctx, Net.UDP.Datagram.F_Payload, Length));
