...
is
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Option.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.IPv4.Option.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Option.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.IPv4.Option.Available_Space (Ctx, Fld)
       and then (if Net.IPv4.Option.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.IPv4.Option.Valid_Value (Fld, Val)
       and then Net.IPv4.Option.Valid_Size (Ctx, Fld, Net.IPv4.Option.Field_Size (Ctx, Fld))
       and then Net.IPv4.Option.Available_Space (Ctx, Fld) >= Net.IPv4.Option.Field_Size (Ctx, Fld)
       and then Net.IPv4.Option.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.IPv4.Option.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Option_Class (Ctx : in out Context; Val : Net.IPv4.Option_Class) is
   begin
      Set_Scalar (Ctx, F_Option_Class, Net.IPv4.To_Base_Integer (Val));
   end Set_Option_Class;

   procedure Set_Option_Number (Ctx : in out Context; Val : Net.IPv4.Option_Number) is
   begin
      Set_Scalar (Ctx, F_Option_Number, Net.IPv4.To_Base_Integer (Val));
   end Set_Option_Number;

   procedure Set_Option_Length (Ctx : in out Context; Val : Net.IPv4.Option_Length) is
   begin
      Set_Scalar (Ctx, F_Option_Length, Net.IPv4.To_Base_Integer (Val));
   ...
   begin
      ...
      pragma Assert (Net.IPv4.Option.Valid_Length (Ctx, Net.IPv4.Option.F_Option_Data, Length));
