...
package body Net.RFLX_Scalar_Sequence
with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First => Buffer_First, Buffer_Last => Buffer_Last, First => First, Last => Last, Buffer => Buffer, Sequence_Last => First - 1, State => S_Valid, First_Element => Net.RFLX_Types.Base_Integer'First, Next_Element => Net.RFLX_Types.Base_Integer'First);
      Buffer := null;
   end Initialize;
   ...
   procedure Next (Ctx : in out Context) is
      Last_Bit : constant RFLX_Types.Bit_Index := Ctx.Sequence_Last + Net.RFLX_Types.Bit_Index (Element_Size);
   ...
   begin
      ...
      then
         Ctx.Next_Element := Net.RFLX_Types.Operations.Extract (Ctx.Buffer.all, Buffer_First, Buffer_Last, Offset, Element_Size, RFLX_Types.High_Order_First);
      ...
      Ctx.Sequence_Last := Ctx.Sequence_Last + Net.RFLX_Types.Bit_Index (Element_Size);
   end Next;
   ...
   begin
      Last_Bit := Ctx.Sequence_Last + Net.RFLX_Types.Bit_Index (Element_Size);
      ...
      if
         ...
         and First <= Last
      then
         Net.RFLX_Types.Operations.Insert (To_Base_Int (Value), Ctx.Buffer.all, First, Last, Offset, Element_Size, RFLX_Types.High_Order_First);
      end if;
      if Size (Ctx) = 0 then
