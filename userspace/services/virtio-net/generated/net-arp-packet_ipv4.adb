...
package body Net.ARP.Packet_IPv4
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Hrd =>
             Invalid (Ctx.Cursors (F_Pro)),
          when F_Pro =>
             Invalid (Ctx.Cursors (F_Hln)),
          when F_Hln =>
             Invalid (Ctx.Cursors (F_Pln)),
          when F_Pln =>
             Invalid (Ctx.Cursors (F_Op)),
          when F_Op =>
             Invalid (Ctx.Cursors (F_Sha)),
          when F_Sha =>
             Invalid (Ctx.Cursors (F_Spa)),
          when F_Spa =>
             Invalid (Ctx.Cursors (F_Tha)),
          when F_Tha =>
             Invalid (Ctx.Cursors (F_Tpa)),
          when F_Tpa =>
             True);
   ...
   with
     Pre =>
       ...
       and Net.ARP.Packet_IPv4.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then Net.ARP.Packet_IPv4.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Tpa then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then Net.ARP.Packet_IPv4.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.ARP.Packet_IPv4.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Tpa and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   ...
   with
     Pre =>
       ...
       and then Net.ARP.Packet_IPv4.Valid_Next (Ctx, Fld)
       and then Fld in F_Hrd | F_Pro | F_Hln | F_Pln | F_Op | F_Sha | F_Spa | F_Tha | F_Tpa
       and then Net.ARP.Packet_IPv4.Valid_Value (Fld, Val)
       and then Net.ARP.Packet_IPv4.Valid_Size (Ctx, Fld, Net.ARP.Packet_IPv4.Field_Size (Ctx, Fld))
       and then Net.ARP.Packet_IPv4.Available_Space (Ctx, Fld) >= Net.ARP.Packet_IPv4.Field_Size (Ctx, Fld)
       and then Net.ARP.Packet_IPv4.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.ARP.Packet_IPv4.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Hrd (Ctx : in out Context; Val : Net.ARP.Hardware_Address_Space) is
   begin
      Set_Scalar (Ctx, F_Hrd, Net.ARP.To_Base_Integer (Val));
   end Set_Hrd;

   procedure Set_Pro (Ctx : in out Context; Val : Net.Ethernet.Ether_Type) is
   begin
      Set_Scalar (Ctx, F_Pro, Net.Ethernet.To_Base_Integer (Val));
   end Set_Pro;

   procedure Set_Hln (Ctx : in out Context; Val : Net.ARP.Address_Length) is
   begin
      Set_Scalar (Ctx, F_Hln, Net.ARP.To_Base_Integer (Val));
   end Set_Hln;

   procedure Set_Pln (Ctx : in out Context; Val : Net.ARP.Address_Length) is
   begin
      Set_Scalar (Ctx, F_Pln, Net.ARP.To_Base_Integer (Val));
   end Set_Pln;

   procedure Set_Op (Ctx : in out Context; Val : Net.ARP.Opcode) is
   begin
      Set_Scalar (Ctx, F_Op, Net.ARP.To_Base_Integer (Val));
   end Set_Op;

   procedure Set_Sha (Ctx : in out Context; Val : Net.Ethernet.Address) is
   begin
      Set_Scalar (Ctx, F_Sha, Net.Ethernet.To_Base_Integer (Val));
   end Set_Sha;

   procedure Set_Spa (Ctx : in out Context; Val : Net.IPv4.Address) is
   begin
      Set_Scalar (Ctx, F_Spa, Net.IPv4.To_Base_Integer (Val));
   end Set_Spa;

   procedure Set_Tha (Ctx : in out Context; Val : Net.Ethernet.Address) is
   begin
      Set_Scalar (Ctx, F_Tha, Net.Ethernet.To_Base_Integer (Val));
   end Set_Tha;

   procedure Set_Tpa (Ctx : in out Context; Val : Net.IPv4.Address) is
   begin
      Set_Scalar (Ctx, F_Tpa, Net.IPv4.To_Base_Integer (Val));
   end Set_Tpa;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Hrd := Get_Hrd (Ctx);
      Struct.Pro := Get_Pro (Ctx);
      Struct.Hln := Get_Hln (Ctx);
      Struct.Pln := Get_Pln (Ctx);
      Struct.Op := Get_Op (Ctx);
      Struct.Sha := Get_Sha (Ctx);
      Struct.Spa := Get_Spa (Ctx);
      Struct.Tha := Get_Tha (Ctx);
      Struct.Tpa := Get_Tpa (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Hrd (Ctx, Struct.Hrd);
      Set_Pro (Ctx, Struct.Pro);
      Set_Hln (Ctx, Struct.Hln);
      Set_Pln (Ctx, Struct.Pln);
      Set_Op (Ctx, Struct.Op);
      Set_Sha (Ctx, Struct.Sha);
      Set_Spa (Ctx, Struct.Spa);
      Set_Tha (Ctx, Struct.Tha);
      Set_Tpa (Ctx, Struct.Tpa);
   end To_Context;
