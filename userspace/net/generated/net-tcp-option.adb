...
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Length =>
             Invalid (Ctx.Cursors (F_Max_Seg_Size))
             and Invalid (Ctx.Cursors (F_SACK_Blocks))
             and Invalid (Ctx.Cursors (F_Shift_Cnt))
             and Invalid (Ctx.Cursors (F_TSval)),
          when F_Max_Seg_Size | F_SACK_Blocks | F_Shift_Cnt =>
             True,
          when F_TSval =>
             Invalid (Ctx.Cursors (F_TSecr)),
          when F_TSecr =>
             True);
   ...
   with
     Pre =>
       ...
       and then Net.TCP.Option.Sufficient_Buffer_Length (Ctx, Fld)
       and then not Net.TCP.Option.Composite_Field (Fld)
   is
         ...
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if
                                  Fld = F_Kind
                                  or Fld = F_Length
                                  or Fld = F_Max_Seg_Size
                                  or Fld = F_SACK_Blocks
                                  or Fld = F_Shift_Cnt
                                  or Fld = F_TSecr
                               then
   ...
   with
     Pre =>
       ...
       and then Net.TCP.Option.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.TCP.Option.Available_Space (Ctx, Fld)
       and then (if Net.TCP.Option.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if
                              Fld in F_Kind | F_Length | F_Max_Seg_Size | F_SACK_Blocks | F_Shift_Cnt | F_TSecr
   ...
   with
     Pre =>
       ...
       and then Net.TCP.Option.Valid_Next (Ctx, Fld)
       and then Fld in F_Kind | F_Length | F_Max_Seg_Size | F_Shift_Cnt | F_TSval | F_TSecr
       and then Net.TCP.Option.Valid_Value (Fld, Val)
       and then Net.TCP.Option.Valid_Size (Ctx, Fld, Net.TCP.Option.Field_Size (Ctx, Fld))
       and then Net.TCP.Option.Available_Space (Ctx, Fld) >= Net.TCP.Option.Field_Size (Ctx, Fld)
       and then Net.TCP.Option.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.TCP.Option.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Kind (Ctx : in out Context; Val : Net.TCP.Kind) is
   begin
      Set_Scalar (Ctx, F_Kind, Net.TCP.To_Base_Integer (Val));
   end Set_Kind;

   procedure Set_Length (Ctx : in out Context; Val : Net.TCP.Length) is
   begin
      Set_Scalar (Ctx, F_Length, Net.TCP.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Max_Seg_Size (Ctx : in out Context; Val : Net.TCP.Max_Seg_Size) is
   begin
      Set_Scalar (Ctx, F_Max_Seg_Size, Net.TCP.To_Base_Integer (Val));
   end Set_Max_Seg_Size;

   procedure Set_Shift_Cnt (Ctx : in out Context; Val : Net.TCP.Shift_Cnt) is
   begin
      Set_Scalar (Ctx, F_Shift_Cnt, Net.TCP.To_Base_Integer (Val));
   end Set_Shift_Cnt;

   procedure Set_TSval (Ctx : in out Context; Val : Net.TCP.TSval) is
   begin
      Set_Scalar (Ctx, F_TSval, Net.TCP.To_Base_Integer (Val));
   end Set_TSval;

   procedure Set_TSecr (Ctx : in out Context; Val : Net.TCP.TSecr) is
   begin
      Set_Scalar (Ctx, F_TSecr, Net.TCP.To_Base_Integer (Val));
   end Set_TSecr;

   procedure Set_SACK_Blocks_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_SACK_Blocks, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_SACK_Blocks_Empty;

   procedure Set_SACK_Blocks (Ctx : in out Context; Seq_Ctx : Net.TCP.SACK_Blocks.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (Net.TCP.SACK_Blocks.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_SACK_Blocks, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      Net.TCP.SACK_Blocks.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_SACK_Blocks;

   procedure Initialize_SACK_Blocks_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then Net.TCP.Option.Has_Buffer (Ctx)
       and then Net.TCP.Option.Valid_Next (Ctx, Net.TCP.Option.F_SACK_Blocks)
       and then Net.TCP.Option.Valid_Length (Ctx, Net.TCP.Option.F_SACK_Blocks, Length)
       and then RFLX_Types.To_Length (Net.TCP.Option.Available_Space (Ctx, Net.TCP.Option.F_SACK_Blocks)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_SACK_Blocks)
       and then Field_Size (Ctx, F_SACK_Blocks) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_SACK_Blocks)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SACK_Blocks);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SACK_Blocks) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_SACK_Blocks);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_SACK_Blocks) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_SACK_Blocks_Private;

   procedure Initialize_SACK_Blocks (Ctx : in out Context) is
   begin
      Initialize_SACK_Blocks_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_SACK_Blocks)));
   end Initialize_SACK_Blocks;

   procedure Switch_To_SACK_Blocks (Ctx : in out Context; Seq_Ctx : out Net.TCP.SACK_Blocks.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SACK_Blocks);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_SACK_Blocks);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_SACK_Blocks) then
         Reset_Dependent_Fields (Ctx, F_SACK_Blocks);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      Net.TCP.SACK_Blocks.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_SACK_Blocks;

   procedure Update_SACK_Blocks (Ctx : in out Context; Seq_Ctx : in out Net.TCP.SACK_Blocks.Context) is
      Valid_Sequence : constant Boolean := Net.TCP.Option.Complete_SACK_Blocks (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      Net.TCP.SACK_Blocks.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_SACK_Blocks) := (State => S_Valid, First => Ctx.Cursors (F_SACK_Blocks).First, Last => Ctx.Cursors (F_SACK_Blocks).Last, Value => Ctx.Cursors (F_SACK_Blocks).Value);
      else
         Reset_Dependent_Fields (Ctx, F_SACK_Blocks);
         Ctx.Cursors (F_SACK_Blocks) := (others => <>);
      end if;
   end Update_SACK_Blocks;
