...
is
   ...
   with
     Pre =>
       ...
       and then Net.TCP.Segment.Valid_Next (Ctx, Fld)
       and then Fld in F_Source_Port | F_Destination_Port | F_Sequence_Number | F_Acknowledgment_Number | F_Data_Offset | F_Reserved | F_NS | F_CWR | F_ECN | F_URG | F_ACK | F_PSH | F_RST | F_SYN | F_FIN | F_Window | F_Checksum | F_Urgent_Pointer
       and then Net.TCP.Segment.Valid_Value (Fld, Val)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : Net.TCP.Reserved) is
   begin
      Set_Scalar (Ctx, F_Reserved, Net.TCP.To_Base_Integer (Val));
   end Set_Reserved;
   ...
   procedure Set_Options (Ctx : in out Context; Seq_Ctx : Net.TCP.Options.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (Net.TCP.Options.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Options, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      Net.TCP.Options.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Options;
   ...
   procedure Switch_To_Options (Ctx : in out Context; Seq_Ctx : out Net.TCP.Options.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Options);
   ...
   begin
      ...
      Net.TCP.Options.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Options;

   procedure Update_Options (Ctx : in out Context; Seq_Ctx : in out Net.TCP.Options.Context) is
      Valid_Sequence : constant Boolean := Net.TCP.Segment.Complete_Options (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      Net.TCP.Options.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
