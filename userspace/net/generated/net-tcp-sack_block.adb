...
package body Net.TCP.SACK_Block
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Left_Edge =>
             Invalid (Ctx.Cursors (F_Right_Edge)),
          when F_Right_Edge =>
             True);
   ...
   with
     Pre =>
       ...
       and Net.TCP.SACK_Block.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then Net.TCP.SACK_Block.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Right_Edge then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       Net.TCP.SACK_Block.Has_Buffer (Ctx)
       and then Net.TCP.SACK_Block.Valid_Next (Ctx, Fld)
       and then Net.TCP.SACK_Block.Valid_Value (Fld, Val)
       and then Net.TCP.SACK_Block.Valid_Size (Ctx, Fld, Size)
       and then Size <= Net.TCP.SACK_Block.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then Net.TCP.SACK_Block.Valid_Next (Ctx, Fld)
       and then Fld in F_Left_Edge | F_Right_Edge
       and then Net.TCP.SACK_Block.Valid_Value (Fld, Val)
       and then Net.TCP.SACK_Block.Valid_Size (Ctx, Fld, Net.TCP.SACK_Block.Field_Size (Ctx, Fld))
       and then Net.TCP.SACK_Block.Available_Space (Ctx, Fld) >= Net.TCP.SACK_Block.Field_Size (Ctx, Fld)
       and then Net.TCP.SACK_Block.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (Net.TCP.SACK_Block.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Left_Edge (Ctx : in out Context; Val : Net.TCP.SACK_Edge) is
   begin
      Set_Scalar (Ctx, F_Left_Edge, Net.TCP.To_Base_Integer (Val));
   end Set_Left_Edge;

   procedure Set_Right_Edge (Ctx : in out Context; Val : Net.TCP.SACK_Edge) is
   begin
      Set_Scalar (Ctx, F_Right_Edge, Net.TCP.To_Base_Integer (Val));
   end Set_Right_Edge;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Left_Edge := Get_Left_Edge (Ctx);
      Struct.Right_Edge := Get_Right_Edge (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Left_Edge (Ctx, Struct.Left_Edge);
      Set_Right_Edge (Ctx, Struct.Right_Edge);
   end To_Context;
