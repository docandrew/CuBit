...
/* Grant buffer size per file (1 page) */
#define GRANT_BUF_SIZE  4096
...
static void *alloc_page_buf(void)
{
    /* Allocate PAGE_SIZE + PAGE_SIZE to guarantee alignment */
    void *raw = cubit_sbrk(PAGE_SIZE + PAGE_SIZE);
    if ((long)raw == -1)
        return NULL;
