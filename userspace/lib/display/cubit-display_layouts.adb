...
package body CuBit.Display_Layouts with SPARK_Mode is
   function Confine (Ready : Layout; Point : G.Logical_Point) return Pointer_Position is
      function Distance (P : G.Logical_Point) return Long_Long_Integer is
        (abs (Long_Long_Integer (Point.X) - Long_Long_Integer (P.X)) +
         abs (Long_Long_Integer (Point.Y) - Long_Long_Integer (P.Y)));
      Result : Pointer_Position := (1, G.Confine (Ready.Items (1).Geometry, Point));
      Best : Long_Long_Integer := Distance (Result.Point);
   begin
      for I in 2 .. Ready.Count loop
         pragma Loop_Invariant (Result.Screen < I and then
           G.Contains (Ready.Items (Result.Screen).Geometry, Result.Point));
         declare
            P : constant G.Logical_Point := G.Confine (Ready.Items (I).Geometry, Point);
            D : constant Long_Long_Integer := Distance (P);
         begin
            if D < Best then Result := (I, P); Best := D; end if;
         end;
      end loop;
      return Result;
   end Confine;
