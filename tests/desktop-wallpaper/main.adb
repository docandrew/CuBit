...
procedure Main is
   ...
   procedure Check (Width, Height, Pitch : Positive) is
   ...
   begin
      Desktop_Wallpaper.Render
        (Storage'Address + 32, Width, Height, Pitch);
      ...
      if Width = 2048 and Height = 576 then
         for Y in 0 .. Height - 1 loop
            for X in 0 .. Width - 1 loop
      ...
      if Width = 1024 and Height = 576 then
   ...
   procedure Check_Damage is
      Full, Partial : aliased Storage_Array (0 .. 120 * 96 * 4 - 1) :=
        (others => 16#CD#);
      In_Damage : Boolean;
   begin
      Desktop_Wallpaper.Render (Full'Address, 120, 96, 480);
      Desktop_Wallpaper.Paint
        (Partial'Address, 120, 96, 480, 7, 1, 31, 20);
         ...
         for X in 0 .. 9 loop
            Desktop_Wallpaper.Paint
              (Partial'Address, 120, 96, 480, X * 13, Y * 7,
               Natural'Min (13, 120 - X * 13), Natural'Min (7, 96 - Y * 7));
         end loop;
      end loop;
